...
########################################################
#flag set eTable formatting
########################################################

desc_flag_set10

flagsetslong = rbind.data.frame(desc_flag_set1,
                                desc_flag_set2,
                                desc_flag_set3,
                                desc_flag_set4,
                                desc_flag_set5,
...
flagsetslong$Parameter = NA
flagsetslong$Parameter[flagsetslong$name=="dense_units1"] = "Layer 1 Dense Units"
flagsetslong$Parameter[flagsetslong$name=="dropout1_proportion"] = "Layer 1 Dropout Parameter"
flagsetslong$Parameter[flagsetslong$name=="L1_penalization1"] = "Layer 1 L1 Penalization"
flagsetslong$Parameter[flagsetslong$name=="dense_units2"] = "Layer 2 Dense Units"
flagsetslong$Parameter[flagsetslong$name=="dropout2_proportion"] = "Layer 2 Dropout Parameter"
flagsetslong$Parameter[flagsetslong$name=="L1_penalization2"] = "Layer 2 L1 Penalization"
flagsetslong$Parameter[flagsetslong$name=="dense_units3"] = "Layer 3 Dense Units"
flagsetslong$Parameter[flagsetslong$name=="dropout3_proportion"] = "Layer 3 Dropout Parameter"
flagsetslong$Parameter[flagsetslong$name=="L1_penalization3"] = "Layer 3 L1 Penalization"
flagsetslong$Parameter[flagsetslong$name=="dense_units4"] = "Layer 4 Dense Units"
flagsetslong$Parameter[flagsetslong$name=="epochs"] = "Epochs"
flagsetslong$Parameter[flagsetslong$name=="batch_size"] = "Batch Size"
flagsetslong$Parameter[flagsetslong$name=="learning_rate"] = "Learning Rate"

with(flagsetslong, table(Parameter, name))

subflagsetslong = subset(flagsetslong, select=c(set, Parameter, value))
colnames(subflagsetslong) = c("Set", "HyperParameter", "Parameter_Value")
head(subflagsetslong)

flagdesc_wide = reshape(subflagsetslong, idvar = "Set", timevar = "HyperParameter", direction = "wide")

write.csv(flagdesc_wide, "eTable3_Tensorflow_ANN_hyperparameter_tunin_sets.csv")
...
save_model_hdf5(bestmodel, "EPLOS_sequential_ann_best_fit.h5", overwrite = TRUE, include_optimizer = TRUE)

h5modload = load_model_hdf5("EPLOS_sequential_ann_best_fit.h5", compile = TRUE)
...
######################################################################
######################################################################
#beta regressions for coefficient associations with optimal classifier
######################################################################
######################################################################
######################################################################
...
#NOTE: break apart outcomes in to separate runs so as to identify 
#problematic associations in beta regression convergence for each outcome

######################################################################
#coefficients for less than 1 month LOS
######################################################################

ascdat_a1 = list()
ascdatall_a1 = list()
for (j in 1){
  for (i in 1:ncol(test_xmatall)){
    ...
    #predfeatdat = subset(cbind.data.frame(pred = bestpreds_probs[,j], feature = test_xmatall[,i]), pred>.0000001)
    tryCatch({
      ...
      #betareg_mod = betareg::betareg(pred ~ feature, data=predfeatdat, link="logit")
      ascdat_a1[[i]] = data.frame(feature = dimnames(test_xmatall)[[2]][i],
    ...
    ascdat_a1[[i]]$rho_sign = sign(ascdat_a1[[i]]$rho)
    ascdat_a1[[i]]$beta_sign = sign(ascdat_a1[[i]]$beta)
  }
  ascdatall_a1[[j]] = do.call("rbind.data.frame", ascdat_a1)
  ascdatall_a1[[j]]$Outcome = dimnames(bestpreds_probs)[[2]][j]
}

######################################################################
#coefficients for 1-2 month LOS
######################################################################
...
for (j in 2){
  for (i in 1:ncol(test_xmatall)){
    ...
    tryCatch({
      ...
      betareg_mod = suppressWarnings(betareg::betareg(bestpreds_probs[,j]~test_xmatall[,i], link="logit"))
      #betareg_mod = betareg::betareg(pred ~ feature, data=predfeatdat, link="logit")
      ascdat_b2[[i]] = data.frame(feature = dimnames(test_xmatall)[[2]][i],
                                  rho = cor(bestpreds_probs[,j], test_xmatall[,i], method="spearman"),
                                  #beta = summary(glm(boot::logit(bestpreds_probs[,j])~test_xmatall[,i], family=gaussian))$coefficients[2,1])
                                  beta = summary(glm(VGAM::cloglog(bestpreds_probs[,j], bvalue=1e-45)~test_xmatall[,i], family=gaussian))$coefficients[2,1],
                                  betaregOR = exp(betareg_mod$coefficients$mean[2])
    ...
    ascdat_b2[[i]]$rho_sign = sign(ascdat_b2[[i]]$rho)
    ascdat_b2[[i]]$beta_sign = sign(ascdat_b2[[i]]$beta)
  }
  ascdatall_b2[[j]] = do.call("rbind.data.frame", ascdat_b2)
  ascdatall_b2[[j]]$Outcome = dimnames(bestpreds_probs)[[2]][j]
}

######################################################################
#coefficients for 2-3 month LOS
######################################################################
...
for (j in 3){
  for (i in 1:ncol(test_xmatall)){
    ...
    tryCatch({
      ...
      betareg_mod = suppressWarnings(betareg::betareg(bestpreds_probs[,j]~test_xmatall[,i], link="logit"))
      #betareg_mod = betareg::betareg(pred ~ feature, data=predfeatdat, link="logit")
      ascdat_c3[[i]] = data.frame(feature = dimnames(test_xmatall)[[2]][i],
                                  rho = cor(bestpreds_probs[,j], test_xmatall[,i], method="spearman"),
                                  #beta = summary(glm(boot::logit(bestpreds_probs[,j])~test_xmatall[,i], family=gaussian))$coefficients[2,1])
                                  beta = summary(glm(VGAM::cloglog(bestpreds_probs[,j], bvalue=1e-45)~test_xmatall[,i], family=gaussian))$coefficients[2,1],
                                  betaregOR = exp(betareg_mod$coefficients$mean[2])
    ...
    ascdat_c3[[i]]$rho_sign = sign(ascdat_c3[[i]]$rho)
    ascdat_c3[[i]]$beta_sign = sign(ascdat_c3[[i]]$beta)
  }
  ascdatall_c3[[j]] = do.call("rbind.data.frame", ascdat_c3)
  ascdatall_c3[[j]]$Outcome = dimnames(bestpreds_probs)[[2]][j]
}

######################################################################
#coefficients for 3-4 month LOS
######################################################################
...
for (j in 4){
  for (i in 1:ncol(test_xmatall)){
    ...
    tryCatch({
      ...
      betareg_mod = suppressWarnings(betareg::betareg(bestpreds_probs[,j]~test_xmatall[,i], link="logit"))
      #betareg_mod = betareg::betareg(pred ~ feature, data=predfeatdat, link="logit")
      ascdat_d4[[i]] = data.frame(feature = dimnames(test_xmatall)[[2]][i],
                                  rho = cor(bestpreds_probs[,j], test_xmatall[,i], method="spearman"),
                                  #beta = summary(glm(boot::logit(bestpreds_probs[,j])~test_xmatall[,i], family=gaussian))$coefficients[2,1])
                                  beta = summary(glm(VGAM::cloglog(bestpreds_probs[,j], bvalue=1e-45)~test_xmatall[,i], family=gaussian))$coefficients[2,1],
                                  betaregOR = exp(betareg_mod$coefficients$mean[2])
    ...
    ascdat_d4[[i]]$rho_sign = sign(ascdat_d4[[i]]$rho)
    ascdat_d4[[i]]$beta_sign = sign(ascdat_d4[[i]]$beta)
  }
  ascdatall_d4[[j]] = do.call("rbind.data.frame", ascdat_d4)
  ascdatall_d4[[j]]$Outcome = dimnames(bestpreds_probs)[[2]][j]
}


######################################################################
#coefficients for 4-5 month LOS
######################################################################
...
for (j in 5){
  for (i in 1:ncol(test_xmatall)){
    ...
    tryCatch({
      ...
      betareg_mod = suppressWarnings(betareg::betareg(bestpreds_probs[,j]~test_xmatall[,i], link="logit"))
      #betareg_mod = betareg::betareg(pred ~ feature, data=predfeatdat, link="logit")
      ascdat_e5[[i]] = data.frame(feature = dimnames(test_xmatall)[[2]][i],
                                  rho = cor(bestpreds_probs[,j], test_xmatall[,i], method="spearman"),
                                  #beta = summary(glm(boot::logit(bestpreds_probs[,j])~test_xmatall[,i], family=gaussian))$coefficients[2,1])
                                  beta = summary(glm(VGAM::cloglog(bestpreds_probs[,j], bvalue=1e-45)~test_xmatall[,i], family=gaussian))$coefficients[2,1],
                                  betaregOR = exp(betareg_mod$coefficients$mean[2])
    ...
    ascdat_e5[[i]]$rho_sign = sign(ascdat_e5[[i]]$rho)
    ascdat_e5[[i]]$beta_sign = sign(ascdat_e5[[i]]$beta)
  }
  ascdatall_e5[[j]] = do.call("rbind.data.frame", ascdat_e5)
  ascdatall_e5[[j]]$Outcome = dimnames(bestpreds_probs)[[2]][j]
}


######################################################################
#coefficients for 5-6 or more months
######################################################################
...
for (j in 6){
  for (i in 1:ncol(test_xmatall)){
    ...
    tryCatch({
      ...
      betareg_mod = suppressWarnings(betareg::betareg(bestpreds_probs[,j]~test_xmatall[,i], link="logit"))
      #betareg_mod = betareg::betareg(pred ~ feature, data=predfeatdat, link="logit")
      ascdat_f6[[i]] = data.frame(feature = dimnames(test_xmatall)[[2]][i],
                                  rho = cor(bestpreds_probs[,j], test_xmatall[,i], method="spearman"),
                                  #beta = summary(glm(boot::logit(bestpreds_probs[,j])~test_xmatall[,i], family=gaussian))$coefficients[2,1])
                                  beta = summary(glm(VGAM::cloglog(bestpreds_probs[,j], bvalue=1e-45)~test_xmatall[,i], family=gaussian))$coefficients[2,1],
                                  betaregOR = exp(betareg_mod$coefficients$mean[2])
    ...
    ascdat_f6[[i]]$rho_sign = sign(ascdat_f6[[i]]$rho)
    ascdat_f6[[i]]$beta_sign = sign(ascdat_f6[[i]]$beta)
  }
  ascdatall_f6[[j]] = do.call("rbind.data.frame", ascdat_f6)
  ascdatall_f6[[j]]$Outcome = dimnames(bestpreds_probs)[[2]][j]
}

######################################################################
#coefficients for 6+ month stay
######################################################################

#problematic are items 97, 

ascdat_g7 = list()
ascdatall_g7 = list()
for (j in 7){
  for (i in c(1:96,98:178)){
    print(paste0("################## currently running outcome number ", j, " WITH predictor number ", i, " ##################"))
    print(paste0("################## currently running outcome =  ", dimnames(bestpreds_probs)[[2]][j], " WITH predictor = ", dimnames(test_xmatall)[[2]][i], " ##################"))
    #predfeatdat = subset(cbind.data.frame(pred = bestpreds_probs[,j], feature = test_xmatall[,i]), pred>.0000001)
    tryCatch({
      ...
      betareg_mod = suppressWarnings(betareg::betareg(bestpreds_probs[,j]~test_xmatall[,i], link="logit"))
      #betareg_mod = betareg::betareg(pred ~ feature, data=predfeatdat, link="logit")
      ascdat_g7[[i]] = data.frame(feature = dimnames(test_xmatall)[[2]][i],
                                  rho = cor(bestpreds_probs[,j], test_xmatall[,i], method="spearman"),
                                  #beta = summary(glm(boot::logit(bestpreds_probs[,j])~test_xmatall[,i], family=gaussian))$coefficients[2,1])
                                  beta = summary(glm(VGAM::cloglog(bestpreds_probs[,j], bvalue=1e-45)~test_xmatall[,i], family=gaussian))$coefficients[2,1],
                                  betaregOR = exp(betareg_mod$coefficients$mean[2])
    ...
    ascdat_g7[[i]]$rho_sign = sign(ascdat_g7[[i]]$rho)
    ascdat_g7[[i]]$beta_sign = sign(ascdat_g7[[i]]$beta)
  }
  ascdatall_g7[[j]] = do.call("rbind.data.frame", ascdat_g7)
  ascdatall_g7[[j]]$Outcome = dimnames(bestpreds_probs)[[2]][j]
}
...
# ascdat = list()
# ascdatall = list()
# for (j in 5:ncol(bestpreds_probs)){
#   for (i in c(1:11,13:59,61:64,66:101,103:105,107:118, 120:122,124:128,130:143,145:153,155:178)){
#     print(paste0("################## currently running outcome number ", j, " WITH predictor number ", i, " ##################"))
#     print(paste0("################## currently running outcome =  ", dimnames(bestpreds_probs)[[2]][j], " WITH predictor = ", dimnames(test_xmatall)[[2]][i], " ##################"))
#     #predfeatdat = subset(cbind.data.frame(pred = bestpreds_probs[,j], feature = test_xmatall[,i]), pred>.0000001)
#     tryCatch({
...
betacoefs_a1 = do.call("rbind.data.frame", ascdatall_a1)
betacoefs_b2 = do.call("rbind.data.frame", ascdatall_b2)
betacoefs_c3 = do.call("rbind.data.frame", ascdatall_c3)
betacoefs_d4 = do.call("rbind.data.frame", ascdatall_d4)
betacoefs_e5 = do.call("rbind.data.frame", ascdatall_e5)
betacoefs_f6 = do.call("rbind.data.frame", ascdatall_f6)
betacoefs_g7 = do.call("rbind.data.frame", ascdatall_g7)

corasc = rbind.data.frame(betacoefs_a1, betacoefs_b2, betacoefs_c3, betacoefs_d4, betacoefs_e5, betacoefs_f6, betacoefs_g7)
                 
corasc = subset(corasc, select=c(Outcome, feature, betaregOR))
glimpse(corasc)

#log ORs for effect size beta sullivan scoring
corasc$logbetareg = log(corasc$betaregOR)
corasc$logbetareg[corasc$logbetareg==-Inf] = NA
corasc$logbetareg[corasc$logbetareg<=-30] = NA
corasc$logbetareg[corasc$logbetareg>=20] = NA
corasc$neg_logbetareg = ifelse(corasc$logbetareg<0 & corasc$logbetareg>=-30, corasc$logbetareg, NA)
corasc$pos_logbetareg = ifelse(corasc$logbetareg>0 & corasc$logbetareg<Inf, corasc$logbetareg, NA)
...
subcorasc = subset(corasc, is.na(logbetareg)==FALSE)

magplot = ggplot(data=subcorasc, group=1) +
...
magplot
ggsave("eFigure1_EPLOS_magnitude_of_effects_beta_ANN_classifier.pdf", magplot, width=11, height=8, scale=1.2)
...
##############################################################
##############################################################
#Sullivan point scoring adaptation, 
...
namedfeats = merge(
  merge(subcorasc, subset(dxnames, select=c(icdcode, icd)), by.x=c("feature"), by.y=c("icdcode"), all.x=TRUE), 
  subset(pxnames, select=c(pxicdcode, pxicd)), by.x=c("feature"), by.y=c("pxicdcode"), all.x=TRUE)
dput(names(namedfeats))
namedfeats$label = coalesce(as.character(namedfeats$icd), as.character(namedfeats$pxicd))
namedfeats$code_type = gsub( "_.*$", "", namedfeats$feature)

featdat = subset(namedfeats, select=c("Outcome", "code_type", "feature", "label",  
...
#######################################################################
#selection of top DXs and PXs per multinomial outcome category
#######################################################################

dput(levels(factor(subcorasc$Outcome)))
...
sullivan_scoring_formatter = function(DATA, DIVISOR=1){
  DATA$sullivan_ref = min(ifelse(DATA$logbetareg>0, DATA$logbetareg, NA), na.rm=TRUE)
...
write.csv(sullscoredat, "eTable4_sullivan_scoring_by_outcome_and_predictive_feature.csv")
...
ggsave("Figure1_disciminations_EPLOS_combined.pdf", ardiscrimplots, width=8, height=8, scale=1.2)

###########################################################################
###########################################################################
###########################################################################
#Sullivan adapted scoring for betareg calibrations
###########################################################################
###########################################################################
###########################################################################
...
arcalibsplots
ggsave("Figure2_calibrations_EPLOS_discrimination.pdf", arcalibsplots, width=8, height=8, scale=1.5)
...
# 
# ggplot(distsulls) +
#   geom_smooth(aes(x=score, y=prcntl_rank, colour=Outcome), method = "gam", formula = y ~ s(x, k=5), se = FALSE, size=1) +
...
markov_binary_ANN_full_model_sim_scoring = function(SIMULATED_CASES, MARG_PROBS, MARG_COVAR, MODEL, QUANTILES, COLNAMES){
  ...
  modpreds = data.frame(predict_proba(MODEL, probmat)) #model predictions from Keras 
...
percannplot = ggplot(markov_annsub) +
  #geom_hline(yintercept=0, colour="black", size=.3) +
...
markov_quantplots = gridExtra::grid.arrange(percannplot, percsullplot, ncol=1)
markov_quantplots
ggsave("Figure3_markov_quantile_plots_arranged.pdf", markov_quantplots, width=8, height=8, scale=1.5)

##############################################################
#create csv of percentile ranks for understanding the risk score percential a patient has
##############################################################

#1. Which tools were you using (Full ANN scoring model, Simplified Point Scoring System of 20 Variables?)
#2. What Length of Stay Outcome Category Did you Score for?
#3. What was the score for the patient?
#Return the Risk Quantile the patient is in.

percsimsall$Tool = "Simplified Point Scoring System"
markov_annsub$Tool = "Full ANN Scoring Model"

modquant_markovs = subset(rbind.data.frame(percsimsall, markov_annsub),
                          select=c(Tool, description, quantile, mean_score))
colnames(modquant_markovs) = c("Risk Tool", "Length of Stay Category", "Quantile", "Patient Score on Tool")
modquant_markovs$Quantile[modquant_markovs$Quantile=="100%"] = "99.9%"
head(modquant_markovs)

write.csv(modquant_markovs, "markov_quantiles_of_risk.csv")
